...
Derived from IBM AMLSim and modified for the curated graph configurations in
this repository. Distributed under the Apache License 2.0; see LICENSE and
...
# Thesis combined v07 coverage calibration.  The first pass gives every account
# a main role; the second pass adds same-bank top-ups for sparse accounts.
COVERAGE_TOPUP_TARGET_MAIN_MODELS = 4
COVERAGE_TOPUP_MAX_GRAPH_DEGREE = 2
COVERAGE_CANDIDATE_SAMPLE_SIZE = 256
...
def allocate_counts(total, capacities):
    total_capacity = sum(capacities.values())
    if total_capacity == 0:
        return {k: 0 for k in capacities}

    total = min(total, total_capacity)
    allocations = {}
    remainders = []
    allocated = 0
    for key, capacity in capacities.items():
        raw = total * capacity / total_capacity
        count = min(int(raw), capacity)
...
def pop_non_self(stubs, src):
    if not stubs:
        return None
    for idx in range(len(stubs) - 1, -1, -1):
        if stubs[idx] != src:
            return stubs.pop(idx)
    return None
...
def directed_configuration_model_by_bank(_in_deg, _out_deg, bank_ids, internal_edge_ratio, seed=0):
    if not sum(_in_deg) == sum(_out_deg):
        raise nx.NetworkXError('Invalid degree sequences. Sequences must have equal sums.')
    if len(_in_deg) != len(bank_ids) or len(_out_deg) != len(bank_ids):
        raise nx.NetworkXError('Degree sequence and bank ID lengths must match.')
    if not 0.0 <= internal_edge_ratio <= 1.0:
        raise ValueError("internal_edge_ratio must be within [0.0, 1.0]")
    ...
    in_by_bank = defaultdict(list)
    out_by_bank = defaultdict(list)
    for n in _g.nodes():
        bank_id = bank_ids[n]
        in_by_bank[bank_id].extend(_in_deg[n] * [n])
        out_by_bank[bank_id].extend(_out_deg[n] * [n])

    for stubs in list(in_by_bank.values()) + list(out_by_bank.values()):
        rng.shuffle(stubs)

    num_edges = sum(len(stubs) for stubs in out_by_bank.values())
    target_internal_edges = int(round(num_edges * internal_edge_ratio))
    capacities = {
        bank_id: min(len(out_by_bank[bank_id]), len(in_by_bank[bank_id]))
        for bank_id in out_by_bank
    }
    internal_targets = allocate_counts(target_internal_edges, capacities)

    edges = []
    delayed_out_stubs = []
    for bank_id, target in internal_targets.items():
        made = 0
        while made < target and out_by_bank[bank_id] and in_by_bank[bank_id]:
            src = out_by_bank[bank_id].pop()
            dst = pop_non_self(in_by_bank[bank_id], src)
            if dst is None:
                delayed_out_stubs.append(src)
                continue
            edges.append((src, dst))
            made += 1

    remaining_out_stubs = delayed_out_stubs
    for stubs in out_by_bank.values():
        remaining_out_stubs.extend(stubs)
    rng.shuffle(remaining_out_stubs)

    def pop_cross_or_any_dst(src):
        src_bank = bank_ids[src]
        cross_candidates = [(b, len(stubs)) for b, stubs in in_by_bank.items() if b != src_bank and stubs]
        if cross_candidates:
            return in_by_bank[weighted_choice(rng, cross_candidates)].pop()

        same_bank_dst = pop_non_self(in_by_bank[src_bank], src)
        if same_bank_dst is not None:
            return same_bank_dst

        any_candidates = [(b, len(stubs)) for b, stubs in in_by_bank.items() if stubs]
        if any_candidates:
            return in_by_bank[weighted_choice(rng, any_candidates)].pop()
        return None

    for src in remaining_out_stubs:
        dst = pop_cross_or_any_dst(src)
        if dst is not None:
            edges.append((src, dst))
...
class TransactionGenerator:
    ...
    def __init__(self, conf, sim_name=None):
        ...
        self.internal_edge_ratio = parse_float(other_conf.get("internal_edge_ratio"))
        self.sar_extra_edges_per_member = max(0.0, parse_float(other_conf.get("sar_extra_edges_per_member")) or 0.0)
        ...
        self.sar_extra_edges_added = 0  # Additional suspicious edges added within SAR groups
    ...
    def get_account_bank_ids(self):
        ...
        with open(self.acct_file, "r") as rf:
            ...
            idx_bank = header.index('bank_id')
            for row in reader:
                if row[0].startswith("#"):
                    continue
                bank_id = row[idx_bank] or self.default_bank_id
                bank_ids.extend([bank_id] * int(row[idx_count]))

        if len(bank_ids) != self.num_accounts:
            raise ValueError("Generated %d bank IDs for %d accounts" % (len(bank_ids), self.num_accounts))
        return bank_ids
    ...
    def generate_normal_transactions(self):
        ...
        degree_pairs = list(zip(in_deg, out_deg))
        random.Random(self.seed).shuffle(degree_pairs)
        in_deg, out_deg = map(list, zip(*degree_pairs))
        if self.internal_edge_ratio is None:
            G = directed_configuration_model(in_deg, out_deg, self.seed)
        else:
            bank_ids = self.get_account_bank_ids()
            if bank_ids is None:
                raise ValueError("internal_edge_ratio requires aggregated accounts with bank_id values")
            logger.info("Target internal base transaction ratio: %f", self.internal_edge_ratio)
            G = directed_configuration_model_by_bank(in_deg, out_deg, bank_ids, self.internal_edge_ratio, self.seed)
        G = nx.DiGraph(G)
        self.g = G
        ...
        while(self.nominator.has_more()):
            for type in self.nominator.types():
                ...
                if count > 0:
                    ...
                    self.normal_model_id += 1
        self.add_missing_main_account_coverage_models()
    ...
    def choose_low_membership_neighbor(
            self,
            node_id,
            candidate_ids,
            preferred_bank_id=None,
            excluded_ids=None,
            sample_size=None):
        ...
        if sample_size is not None and len(candidate_pool) > sample_size:
            candidate_pool = random.sample(candidate_pool, sample_size)
        candidates = [
            candidate for candidate in candidate_pool
            if candidate != node_id and candidate not in excluded_ids
        ]
        if preferred_bank_id is not None:
            same_bank_candidates = [
                candidate for candidate in candidates
                if self.acct_to_bank.get(candidate) == preferred_bank_id
            ]
            if same_bank_candidates:
                candidates = same_bank_candidates
        if not candidates:
            return None
        return min(
            candidates,
            key=lambda candidate: (
                len(self.g.node[candidate]['normal_models']),
                self.g.degree(candidate),
                candidate,
            ),
        )

    def add_missing_main_account_coverage_models(self):
        main_ids = {normal_model.main_id for normal_model in self.normal_models}
        main_model_counts = Counter(normal_model.main_id for normal_model in self.normal_models)
        all_account_ids = sorted(self.g.nodes())
        bank_account_lists = {
            bank_id: sorted(account_ids)
            for bank_id, account_ids in self.bank_to_accts.items()
        }
        added_models = 0
        added_edges = 0
        topup_models = 0
        ...
        for node_id in sorted(self.g.nodes()):
            ...
            bank_id = self.acct_to_bank.get(node_id)
            successor = self.choose_low_membership_neighbor(node_id, self.g.successors(node_id), bank_id)
            if successor is None:
                successor = self.choose_low_membership_neighbor(node_id, self.g.predecessors(node_id), bank_id)
                if successor is None:
                    successor = self.choose_low_membership_neighbor(
                        node_id,
                        bank_account_lists.get(bank_id, all_account_ids),
                        bank_id,
                        sample_size=COVERAGE_CANDIDATE_SAMPLE_SIZE,
                    )
                if successor is None:
                    successor = self.choose_low_membership_neighbor(
                        node_id,
                ...
                if successor is None:
                    continue
                if not self.g.has_edge(node_id, successor):
                    self.g.add_edge(node_id, successor)
                    self.add_edge_info(node_id, successor)
                    added_edges += 1

            result_ids = {node_id, successor}
            self.add_coverage_normal_model(node_id, result_ids)
            added_models += 1
            main_model_counts[node_id] += 1
            if bank_id is not None and self.acct_to_bank.get(successor) != bank_id:
                cross_bank_models += 1

        for node_id in sorted(self.g.nodes()):
            if self.g.degree(node_id) > COVERAGE_TOPUP_MAX_GRAPH_DEGREE:
                continue

            bank_id = self.acct_to_bank.get(node_id)
            used_counterparts = set()
            while main_model_counts[node_id] < COVERAGE_TOPUP_TARGET_MAIN_MODELS:
                counterpart = self.choose_low_membership_neighbor(
                    node_id,
                    bank_account_lists.get(bank_id, all_account_ids),
                    bank_id,
                    used_counterparts,
                ...
                if counterpart is None:
                    counterpart = self.choose_low_membership_neighbor(
                        node_id,
                        all_account_ids,
                        excluded_ids=used_counterparts,
                        sample_size=COVERAGE_CANDIDATE_SAMPLE_SIZE,
                    )
                if counterpart is None:
                    break

                used_counterparts.add(counterpart)
                if not self.g.has_edge(node_id, counterpart):
                    self.g.add_edge(node_id, counterpart)
                    self.add_edge_info(node_id, counterpart)
                    topup_edges += 1

                self.add_coverage_normal_model(node_id, {node_id, counterpart})
                main_model_counts[node_id] += 1
                topup_models += 1
                if bank_id is not None and self.acct_to_bank.get(counterpart) != bank_id:
                    cross_bank_models += 1

        logger.info(
            "Added %d one-shot coverage normal models for accounts without a main role (%d fallback edges).",
            added_models,
            added_edges,
        )
        logger.info(
            "Added %d same-bank low-activity top-up normal models (%d fallback edges, %d cross-bank coverage models).",
            topup_models,
            topup_edges,
            cross_bank_models,
        )

    def add_coverage_normal_model(self, main_id, result_ids):
        normal_model = NormalModel(self.normal_model_id, 'single', result_ids, main_id)
    ...
    def load_alert_patterns(self):
        ...
        if self.sar_extra_edges_per_member > 0.0:
            logger.info(
                "Added %d extra suspicious intra-group edges via sar_extra_edges_per_member=%.2f",
                self.sar_extra_edges_added,
                self.sar_extra_edges_per_member,
            )
    ...
    def add_aml_typology(self, is_sar, typology_name, num_accounts, min_amount, max_amount, period, bank_id="", schedule=1):
        ...
        def add_main_acct(preferred_bank_id=None):
            """Create a main account ID and a bank ID from hub accounts
            :return: main account ID and bank ID
            """
            if preferred_bank_id is None:
                self.check_hub_exists()
                main_candidates = self.hubs
            else:
                main_candidates = self.hubs & self.bank_to_accts[preferred_bank_id]
                if not main_candidates:
                    raise ValueError("No main account candidates found for bank ID: %s" % preferred_bank_id)

            _main_acct = random.sample(main_candidates, 1)[0]
        ...
        def add_extra_sar_edges():
            """Add extra suspicious edges among the same SAR members.
            This raises suspicious edge density without changing the SAR node set.
            """
            if not is_sar or self.sar_extra_edges_per_member <= 0.0:
                return
            ...
            existing_pairs = set(sub_g.edges())
            candidate_pairs = [
                (orig, bene)
                for orig in members
                for bene in members
                if orig != bene and (orig, bene) not in existing_pairs
            ]
            if not candidate_pairs:
                return

            target_extra_edges = max(1, int(round(len(members) * self.sar_extra_edges_per_member)))
            target_extra_edges = min(target_extra_edges, len(candidate_pairs))
            random.shuffle(candidate_pairs)

            for orig, bene in candidate_pairs[:target_extra_edges]:
        ...
        if bank_id == "" and len(self.bank_to_accts) >= 2:
            ...
            is_external = False
        preferred_main_bank_id = None if is_external or bank_id == "" else bank_id
        ...
        if typology_name == "fan_in":  # fan_in pattern (multiple accounts --> single (main) account)
            main_acct, main_bank_id = add_main_acct(preferred_main_bank_id)
            ...
            orig_bank_id = preferred_main_bank_id or random.choice(self.get_all_bank_ids())
            if is_external:
            ...
            if is_external:
                ...
                orig_bank_id = mid_bank_id = bene_bank_id = preferred_main_bank_id or random.sample(self.get_all_bank_ids(), 1)[0]
